...
function validateMajor() {
  ...
  if (majorInput.value.trim() === "") {
    errorElement.textContent = "Please enter your major.";
    return false;
  } else {
    errorElement.textContent = ""; // Clear the error message when valid
  }
...
function validateActivity() {
  ...
  if (activityInput.value.trim() === "") {
    errorElement.textContent = "Please enter the activity.";
    return false;
  } else {
    errorElement.textContent = ""; // Clear the error message when valid
  }
...
function validateFormOnInput() {
  ...
  validateStudentID();
  validateEmail();
  validateMajor();    // Add this line
  validateActivity(); // Add this line
}
...
async function submitForm(event) {
  ...
  const data = {
    ...
    title: formData.get("major"),
    title2: formData.get("activity"),
...
function handleInputChange(event) {
  if (validateFormOnInput()) {
    // Your code to handle input change events when validation passes
  }
}
...
document.getElementById("major").addEventListener("input", validateMajor);
